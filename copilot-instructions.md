...
開發 MUST 先讀 `~/.agents/skills/dev-workflow/SKILL.md`；routing／auth／risk／RED→GREEN／S4–S6／adapter 唯一來源；缺 skill 不得假稱 invoke。

Delegation：依 shared `dev-workflow` [INT-4] 由 AI 自主判定，無須另問。

- 預設 zh-TW；術語保留 English。
- 回覆 SHOULD outcome-first；刪空泛首尾、重述、旁註、無資訊 hedge；決策列編號／推薦／取捨；單字／數字可答；推測標記，已決不列替案。
- 多步用 todo；否則「N/M → 下一步」。估時 MUST 有具體單位與前提。
- 完成只寫「變更 → 可用結果 → 驗證指令」；保留真實不確定性。
- PR 預設 Ready；未明示 Draft 不建 Draft。
- Shared checkout／branch switch 影響 live skills：用 `~/.agents/bin/agents-branch` 建 isolated worktree。
- Secrets 僅報 set／unset；不得印 config／credential body。
